sap.ui.define([
	...
	return BaseObject.extend("dma.zfichatec.model.GroupSortState", {
		 ...
		 * @alias dma.zfichatec.model.GroupSortState
		 */
		...
		/**
		 * Groups by Sugestao, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Sugestao") {
				...
				this._oViewModel.setProperty("/sortBy", "Sugestao");

				aSorters.push(
					new Sorter("Sugestao", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Maktx");
			}
